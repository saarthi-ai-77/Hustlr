...
const AuthPage = () => {
  ...
  const handleSocialLogin = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      // Supabase will redirect the window, so we don't need to navigate manually here
    } catch (error) {
      toast.error(`${provider} login failed`, {
        description: error instanceof Error ? error.message : "Alternative sign-in failed"
      });
      setIsLoading(false);
    }
  };
